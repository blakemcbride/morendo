...
\item The \fn{logical} conditional element is not supported.
  ...
  queries; \fn{(not (and ...))} is likewise a rule element only.
...
\item \fn{generate-macro} writes sources that must be compiled before
  \fn{use-macro} can load them, and loaded macros do not yet replace
  reflection when facts are read (Chapter~\ref{ch:java-objects}).
\item \fn{chaining-direction} is recorded but the engine always chains
  forward.
\item A variable first bound inside a \fn{forall} or a
  \fn{(not (and ...))} is local to it; the rest of the rule cannot use it.
