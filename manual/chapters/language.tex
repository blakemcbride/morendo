...
followed by slot constraints. Slots not mentioned are unconstrained.
...
\item[\fn{(not (pattern))}] Holds when no fact matches the pattern. The
  ...
  for the facts bound so far when the call is false.
\item[\fn{(test (expr))}] Holds when the function call is true. Variables
  used in it must be bound by earlier patterns.
\item[\fn{(exists (pattern)...)}] Holds when at least one combination of
  ...
  many there are. \file{samples/exists/} has fifteen variations.
\item[\fn{(only (pattern)...)}] Holds when \emph{exactly one} combination
  matches, and binds its variables. In the first sample of
