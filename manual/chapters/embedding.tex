...
Or bundle several functions in a \fn{FunctionGroup} and list the group
in the jar's service file (\file{META-INF/services/}, file
\file{org.morendo.rete.FunctionGroup}) so that every engine picks it up
from the class path. The example beans' module shows a
\fn{HelloFunction} written this way.
...
container, \fn{org.morendo.service.servlet.RuleStartupService} is
registered as a listener in \file{web.xml}, reads
\file{/WEB-INF/Ruleconfig.json}, builds the pools and publishes the
service as a servlet-context attribute under the service name.
...
name and version (null when none was given). Initial data of the
