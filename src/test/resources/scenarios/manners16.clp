;; Golden scenario: the Manners benchmark rules (copied from benchmark/manners/manners16.clp)
;; with the 16-guest data set, minus the profiling commands so the output is deterministic.
(deftemplate guest
   (slot name) 
   (slot sex)
   (slot hobby) )
...
(assert (count (c 1) ) )
(load-facts ./benchmark/manners/16guest.dat)
