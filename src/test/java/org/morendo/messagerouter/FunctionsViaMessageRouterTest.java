...
public class FunctionsViaMessageRouterTest {
        ...
        do {
            ...
            for (MessageEvent message : messages) {
                if (message.isError()) {
                    ...
                    Pattern p =
                            Pattern.compile("(for a total of )([0-9]+)", Pattern.CASE_INSENSITIVE);
                    Matcher m = p.matcher(output);
                    while (m.find()) {
                        int facts = Integer.valueOf(m.group(2)) - 1;
                        System.out.println(
                                "Vergleich: Fakten in RuleML = "
                                        + facts
                                        + ", Fakten hinzugefügt = "
                                        + loop);
    ...
    public void test_rule_slots() {
        ...
        loop = 10;
        for (int i = 1; i <= loop; ++i) {
            stringChannel.executeCommand(
                    "(assert(ass_tst" + i + "(name tst" + i + ")(size " + i + ")))");
            Thread.yield();
            stringChannel.fillEventList(messages);
            messages.clear();
        }
