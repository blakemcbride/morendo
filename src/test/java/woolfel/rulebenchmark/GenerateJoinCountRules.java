...
public class GenerateJoinCountRules {
    ...
    public void write20JoinRule(int count, StringBuilder buf) {
        for (int idx=1; idx <= count; idx++) {
            ...
            buf.append("    (stringfield12 " + strvar12 + ")" +
                    Constants.LINEBREAK);
            buf.append("    (stringfield13 " + strvar13 + ")" +
                    Constants.LINEBREAK);
            buf.append("    (stringfield14 " + strvar14 + ")" +
                    Constants.LINEBREAK);
            buf.append("    (stringfield15 " + strvar15 + ")" +
                    Constants.LINEBREAK);
            buf.append("    (stringfield16 " + strvar16 + ")" +
                    Constants.LINEBREAK);
            buf.append("    (stringfield17 " + strvar17 + ")" +
                    Constants.LINEBREAK);
            buf.append("  )" + Constants.LINEBREAK);
            buf.append("  (object2" + Constants.LINEBREAK);
            buf.append("    (intfield " + intvar + ")" +
