...
/**
 * @author Josef Alexander Hahn ViewGraphNode represents a node in the visualisation graph
 */
public final class ViewGraphNode {
    ...
    protected static ViewGraphNode buildFromRete(
            BaseNode root, HashMap<BaseNode, ViewGraphNode> ht) {
        ...
        if (foo == null) {
            ...
            for (int i = 0; i < succ.length; i++) {
                ...
                if (nxtnode != null && nxtnode.getSuccessorNodes() != null) {
                    res.addToChilds(buildFromRete(nxtnode, ht));
                }
            }
            ht.put(root, res);
        } else {
    ...
    /**
     * Returns the logical width of this node. It is 1 iff whichSubtree is the first parent. else 0
     *
     * @param whichSubtree
     */
    ...
    /**
     * Calculates the width of the subtree in a logical unit. (one node has width=2, two nodes have
     * width=4 and so on; this is because we need "half node widths" for centering one node related
     * to an even numbers of other nodes)
     */
    ...
    /**
     * gets the parentsChecked-Flag used in the visualiser
     *
    ...
    /**
     * sets the parentsChecked-Flag used in the visualiser
     *
