...
/**
 * @author Josef Alexander Hahn This Class represents a connection between two Shapes, which will be
 *     drawn as an arrow
 */
public class ConnectorLine extends Primitive {
    ...
    /**
     * Draws an arrow from the From-Shape to the To-Shape with the choosen color. The draw-position
     * is translated by (-offsetX,-offsetY).
     *
    ...
    /**
     * Draws an arrow from the From-Shape to the To-Shape with the choosen color. It will be scaled
     * by (factorX,factorY).
     *
     * @param canvas The canvas to draw the arrow on
    ...
    /**
     * Draws an arrow from the From-Shape to the To-Shape with the choosen color. The draw-position
     * is translated by (-offsetX,-offsetY) and then scaled by (factorX,factorY).
     *
     * @param canvas The canvas to draw the arrow on
     * @param offsetX Translation-Vector's negative x-component
     * @param offsetY Translation-Vector's negative y-component
    ...
    public void draw(Graphics2D canvas, int offsetX, int offsetY, double factorX, double factorY) {
         ...
         *            FROM
         */
        double angleFromTo = Math.atan2(-to.getY() + from.getY(), to.getX() - from.getX());
        ...
        Point parrow1 =
                new Point(
                        (int) Math.round(6 * Math.cos(angleToFrom + Math.PI / 4.0)),
                        (int) Math.round(6 * Math.sin(angleToFrom + Math.PI / 4.0)));
        Point parrow2 =
                new Point(
                        (int) Math.round(6 * Math.cos(angleToFrom - Math.PI / 4.0)),
                        (int) Math.round(6 * Math.sin(angleToFrom - Math.PI / 4.0)));
