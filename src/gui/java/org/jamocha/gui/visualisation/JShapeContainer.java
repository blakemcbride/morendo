...
/**
 * @author Josef Alexander Hahn A Swing component which helds a list of primitives and draws them
 */
public class JShapeContainer extends JComponent {
    ...
    public Shape getShapeAtPosition(int x, int y) {
        ...
        for (Iterator<Shape> it = shapes.iterator(); it.hasNext(); ) {
            ...
            int offX = (x - s.getX());
            int offY = (y - s.getY());
            if (offX >= 0 && offY >= 0 && offX <= s.getWidth() && offY <= s.getHeight()) return s;
        }
        return null;
    }
}
