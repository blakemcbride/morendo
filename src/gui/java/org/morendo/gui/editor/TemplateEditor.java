package org.morendo.gui.editor;
...
public final class TemplateEditor extends AbstractJamochaEditor implements ActionListener {
    ...
    public TemplateEditor(Rete engine) {
        ...
        reloadButtonDumpAreaTemplate =
                new JButton("Reload Template Preview", IconLoader.getImageIcon("arrow_refresh"));
    ...
    public void actionPerformed(ActionEvent event) {
        if (event.getSource() == assertButton) {
            ...
            EditorRow row =
                    new EditorRow(
                            new DeleteButton(IconLoader.getImageIcon("delete"), rows.size()),
                            new JLabel("Slot " + (rows.size() + 1)),
                            getNewTypesCombo(),
                            new JTextField());
            ...
            addTypesCombo(
                    templatePanel, row.typeBox, gridbag, gridbagConstraints, (rows.size() + 1));
            addNameField(
                    templatePanel, row.nameField, gridbag, gridbagConstraints, (rows.size() + 1));
            rows.add(row);
            ...
            for (int i = 0; i < rows.size(); ++i) {
                ...
                addRemoveButton(
                        templatePanel, editorRow.deleteButton, gridbag, gridbagConstraints, i + 1);
                ...
                addNameField(
                        templatePanel, editorRow.nameField, gridbag, gridbagConstraints, i + 1);
            }
    ...
    private String getCurrentDeftemplateString(boolean print) {
        StringBuilder res =
                new StringBuilder(
                        "(deftemplate " + moduleBox.getSelectedItem() + "::" + nameField.getText());
        if (print) res.append("\n");
        for (EditorRow row : rows) {
            res.append("    (");
            if (row.typeBox.getSelectedItem().toString().equals("MULTISLOT")) {
    ...
    private void addRemoveButton(
            JPanel parent, JButton button, GridBagLayout gridbag, GridBagConstraints c, int row) {
        c.gridx = 0;
        c.gridy = row;
    ...
    private void addLabel(
            JPanel parent, JLabel label, GridBagLayout gridbag, GridBagConstraints c, int row) {
        c.gridx = 1;
        c.gridy = row;
        c.fill = GridBagConstraints.NONE;
        c.anchor = GridBagConstraints.EAST;
    ...
    private void addNameField(
            JPanel parent, JTextField field, GridBagLayout gridbag, GridBagConstraints c, int row) {
        c.gridx = 3;
        c.gridy = row;
