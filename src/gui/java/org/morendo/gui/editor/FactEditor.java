...
import org.morendo.rete.MultiSlot;
...
public final class FactEditor extends AbstractJamochaEditor
    ...
    private void initPreselectionPanel() {
        ...
        preselectionPanel.setBorder(
                BorderFactory.createTitledBorder("Module and Template Selection"));
    ...
    private void initTemplateList() {
        ...
        Module module =
                engine.getWorkingMemory().findModule(String.valueOf(moduleList.getSelectedValue()));
        if (module != null) {
    ...
    private void initFactEditPanel() {
        ...
        if (templateList.getSelectedIndex() > -1) {
            Module module =
                    engine.getWorkingMemory()
                            .findModule(String.valueOf(moduleList.getSelectedValue()));

            Template tmp = module.getTemplate(String.valueOf(templateList.getSelectedValue()));
        ...
        reloadButtondumpAreaFact =
                new JButton("Reload Fact Preview", IconLoader.getImageIcon("arrow_refresh"));
    ...
    private void showCurrentStep() {
        switch (step) {
            case 0:
                ((CardLayout) contentPanel.getLayout()).show(contentPanel, "preselection");
                if (templateList.getSelectedIndex() > -1) {
            ...
            case 1:
                initFactEditPanel();
                ((CardLayout) contentPanel.getLayout()).show(contentPanel, "factEdit");
    ...
    public void valueChanged(ListSelectionEvent event) {
        if (event.getSource() == moduleList.getSelectionModel()) {
            ...
            if (templateList.getSelectedIndex() > -1) {
                ...
                dumpAreaTemplate.setText("(deftemplate " + tmp.getName() + "\n");
                Slot[] slots = (Slot[]) tmp.getAllSlots();
                for (Slot slot : slots) {
    ...
    private String getCurrentFactAssertionString(boolean print) {
        ...
        for (Slot slot : factComponents.keySet()) {
            ...
            if (slot instanceof MultiSlot) {
                Object[] values =
                        ((DefaultListModel<?>) ((JList<?>) currComponent).getModel()).toArray();
    ...
    private final class MultiSlotEditor implements ActionListener, PopupMenuListener {
        ...
        public void actionPerformed(ActionEvent event) {
            if (event.getSource() == addMenuItem) {
                ...
                String value =
                        JOptionPane.showInputDialog("Enter the value:", list.getSelectedValue());
