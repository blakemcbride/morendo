 ...
 */
package org.morendo.gui;

import org.morendo.gui.icons.IconLoader;
import org.morendo.gui.tab.AbstractJamochaPanel;
import org.morendo.gui.tab.FactsPanel;
import org.morendo.gui.tab.FunctionsPanel;
import org.morendo.gui.tab.LogPanel;
import org.morendo.gui.tab.RetePanel;
import org.morendo.gui.tab.SettingsPanel;
import org.morendo.gui.tab.ShellPanel;
import org.morendo.gui.tab.TemplatesPanel;
...
public final class JamochaGui extends JFrame implements ChangeListener {

    static final Preferences preferences = Preferences.userRoot().node("org.morendo.gui");
     ...
     *
     * @param engine The Jamocha-engine that will be used in the GUI.
     */
    public JamochaGui(Rete engine) {
        ...
        tabbedPane.addTab(
                "Shell",
                IconLoader.getImageIcon("application_osx_terminal"),
                shellPanel,
                "Jamocha Shell");
        panels.add(shellPanel);
        FactsPanel factsPanel = new FactsPanel(this);
        tabbedPane.addTab(
                "Facts", IconLoader.getImageIcon("database"), factsPanel, "View or modify Facts");
        panels.add(factsPanel);
        TemplatesPanel templatesPanel = new TemplatesPanel(this);
        tabbedPane.addTab(
                "Templates",
                IconLoader.getImageIcon("brick"),
                templatesPanel,
                "View or modify Templates");
        panels.add(templatesPanel);
        FunctionsPanel functionsPanel = new FunctionsPanel(this);
        tabbedPane.addTab(
                "Functions", IconLoader.getImageIcon("cog"), functionsPanel, "View Functions");
        panels.add(functionsPanel);
        RetePanel retePanel = new RetePanel(this);
        tabbedPane.addTab(
                "Rete", IconLoader.getImageIcon("eye"), retePanel, "View the Rete-network");
        panels.add(retePanel);
        LogPanel logPanel = new LogPanel(this);
        tabbedPane.addTab(
                "Log",
                IconLoader.getImageIcon("monitor"),
                logPanel,
        ...
        tabbedPane.addTab(
                "Settings",
                IconLoader.getImageIcon("wrench"),
                settingsPanel,
                "Settings for Jamocha");
    ...
    /**
     * This sets if only the Gui is closed on exit or also the engine. By default only the Gui will
     * be closed.
     *
     * @param exitOnClose If false, only the gui will be closed
     */
    ...
    public StringChannel getStringChannel() {
        if (stringChannel == null)
            stringChannel =
                    getEngine()
                            .getMessageRouter()
                            .openChannel("gui_string_channel", InterestType.NONE);
    ...
    public void close() {
        if (stringChannel != null) getEngine().getMessageRouter().closeChannel(stringChannel);
