 ...
 * @author Alexander Wilden <october.rust@gmx.de> Modified - Dave Woodman 23/05/21 - send currline
 *     with line end so that comments are recognised
 */
@SuppressWarnings("serial") // Swing components are never serialized here
public final class ShellPanel extends AbstractJamochaPanel
        implements ActionListener, FocusListener, AdjustmentListener {

    /** Flag for the ChannelListener and the eventThread to know if they should go on working. */
    private boolean running = true;

    /**
     * The Queue for incoming KeyEvents. We process them in an own Thread to prevent strange,
    ...
    /**
     * The maximum size of the history. The boundary is to prevent memory problems. A history of
     * less than zero means unbounded history.
     */
    ...
    /**
     * The last position of the Prompt or in case of a new line the position of the beginning of the
     * new line.
     */
    ...
    /**
     * The Timer that makes the Cursor blinking. Set a smaller delay (first parameter) to let it
     * blink faster.
     */
    private Timer cursorTimer = new Timer(400, this);

    /** A Flag indicating if the Shell currently shows the Cursor or the cursorSubString. */
     ...
     *
     * @param engine The Jamocha engine that should be used with this GUI.
     */
    public ShellPanel(JamochaGui gui) {
        ...
        clearButton =
                new JButton("Clear Shell", IconLoader.getImageIcon("application_osx_terminal"));
        ...
        channel = gui.getEngine().getMessageRouter().openChannel("JamochaGui", inStream);
    ...
    /** prints the start up message with copyright and version number */
    private synchronized void printStartMessage() {
        outputArea.append(Constants.PROJECT_MESSAGE + Constants.LINEBREAK);
        outputArea.append(Constants.SHELL_MESSAGE + Constants.LINEBREAK);
    }
     ...
     * @param lineBreak If true a linebreak is added at the end.
     */
     ...
     *
     * @param newPosition The new Position for the Cursor.s
     */
        ...
        if (!cursorShowing) {
            ...
            if (currOffset >= (cursorPosition + SHELL_CURSOR.length())) {
                ...
                outputArea.replaceRange(
                        SHELL_CURSOR, cursorPosition, cursorPosition + cursorSubString.length());
            } else {
    ...
    private synchronized void hideCursor() {
        if (cursorShowing) {
            try {
                outputArea.replaceRange(
                        cursorSubString, cursorPosition, cursorPosition + SHELL_CURSOR.length());
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    private synchronized void startTimer() {
        if (!cursorTimer.isRunning()) cursorTimer.start();
    }
    ...
    /**
     * Initializes and starts the ChannelListener that waits for results or errors from the Jamocha
     * engine.
     */
    private void initChannelListener() {
        channelListener =
                new Thread("morendo-gui-shell") {
                            ...
                            if (!msgEvents.isEmpty()) {
                                ...
                                for (MessageEvent event : msgEvents) {
                                    ...
                                    if (event.getType() == MessageEvent.Type.ERROR) {
                                        buffer.append(
                                                exceptionToString((Exception) event.getMessage())
                                                                .trim()
                                                        + System.getProperty("line.separator"));
                                    }
                                    if (event.getType() != MessageEvent.Type.COMMAND
                                            && !event.getMessage().toString().equals("")
                                            && !event.getMessage().equals(Constants.NIL_SYMBOL)) {
                                        if (event.getMessage() instanceof DefaultReturnVector) {
                                            DefaultReturnVector rv =
                                                    (DefaultReturnVector) event.getMessage();
                    ...
                    /**
                     * Converts an Exception to a String namely turns the StackTrace to a String.
                     *
                                ...
                                switch (e.getKeyCode()) {
                                            ...
                                            try {
                                                try {
                                                    currLine =
                                                            outputArea.getText(
                                                                    lastPromptIndex,
                                                                    getOffset() - lastPromptIndex);
                                                } catch (BadLocationException e1) {
                                                    e1.printStackTrace();
                                                }
                                                lastIncompleteCommand.append(
                                                        currLine
                                                                + System.getProperty(
                                                                        "line.separator"));
                                    ...
                                    default:
                                        if (!e.isControlDown() && !e.isMetaDown()) {
                                            // simple character
                                            printMessage(String.valueOf(e.getKeyChar()), false);
                                        } else {
    ...
    /**
     * A function that safely adds single lines to the history. If a String contains more than one
     * line it will be splitted line by line.
     *
     * <p>If the history size is greater than history_max_size we remove elements that are old and
     * too much.
     *
     * @param historyString The String that should be added to the history.
     */
    ...
    private void initPopupMenu() {
        ...
        copyMenu.addMouseListener(
                new MouseAdapter() {
                    public void mouseReleased(MouseEvent event) {
                        ClipboardUtil.getInstance()
                                .setClipboardContents(outputArea.getSelectedText());
                    }
                });
        JMenuItem pasteMenu = new JMenuItem("Paste", IconLoader.getImageIcon("paste_plain"));
        pasteMenu.addMouseListener(
                new MouseAdapter() {
                    public void mouseReleased(MouseEvent event) {
        ...
        selectCommandMenu.addMouseListener(
                new MouseAdapter() {
                    public void mouseReleased(MouseEvent event) {
        ...
        selectAllMenu.addMouseListener(
                new MouseAdapter() {
                    public void mouseReleased(MouseEvent event) {
    ...
    /**
     * Clears the outputArea and sets a new prompt. If parts of a command are already in the channel
     * these commandparts will be printed again.
     */
    ...
    /**
     * Sets the focus of this panel and by this sets the focus to the outputArea so that the user
     * doesn't have to click on it before he can start typing.
     */
    @Override
    ...
    public void settingsChanged() {
        outputArea.setFont(
                new Font(
                        gui.getPreferences().get("shell.font", "Courier"),
                        gui.getPreferences().getInt("shell.fontstyle", Font.PLAIN),
                        gui.getPreferences().getInt("shell.fontsize", 12)));
        outputArea.setBackground(
                new Color(
                        gui.getPreferences()
                                .getInt("shell.backgroundcolor", Color.BLACK.getRGB())));
        outputArea.setForeground(
                new Color(gui.getPreferences().getInt("shell.fontcolor", Color.WHITE.getRGB())));
    ...
    public void adjustmentValueChanged(AdjustmentEvent event) {
        if (event.getValue() < lastScrollBarPosition) stopTimer();
        else {
