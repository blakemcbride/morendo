...
import org.morendo.gui.TableMap;
import org.morendo.gui.TableSorter;
import org.morendo.gui.editor.TemplateEditor;
...
/**
 * This Panel shows all Templates of all Modules currently in the Engine. You can add new Templates
 * or delete old ones.
 *
...
public final class TemplatesPanel extends AbstractJamochaPanel
    ...
    public TemplatesPanel(JamochaGui gui) {
        ...
        pane =
                new JSplitPane(
                        ...
                        new JScrollPane(templatesTable),
    ...
    private void initTemplatesList() {
        ...
        templatesTable
                .getColumnModel()
                .getColumn(1)
                .setPreferredWidth(templatesTable.getWidth() - 100);
    }
    ...
    private void initPopupMenu() {
        ...
        JMenuItem removeItem =
                new JMenuItem(
                        "Remove selected Template(s)", IconLoader.getImageIcon("brick_delete"));
        removeItem.addMouseListener(
                new MouseAdapter() {
    ...
    public void close() {
        gui.getPreferences().putInt("templates.dividerlocation", pane.getDividerLocation());
    }
    ...
    private final class TemplatesTableModel extends AbstractTableModel {
        ...
        public Class<?> getColumnClass(int aColumn) {
            ...
            else if (aColumn == 1) return java.lang.String.class;
    ...
    public void valueChanged(ListSelectionEvent arg0) {
        if (arg0.getSource() == templatesTable.getSelectionModel()) {
            ...
            if (templatesTable.getSelectedColumnCount() == 1
                ...
                ExtTemplate template = dataModel.getRow(templatesTable.getSelectedRow());
                if (template != null) {
                    buffer.append(
                            "("
                                    + template.getModule().getModuleName()
                                    + "::"
                                    + template.getTemplate().getName());
                    Slot[] slots = (Slot[]) template.getTemplate().getAllSlots();
                    for (Slot slot : slots) {
                        buffer.append("\n    (");
                        if (slot instanceof MultiSlot)
                            ...
                            buffer.append(
                                    "slot "
                                            + slot.getName()
                                            + " (type "
                                            + ConversionUtils.getTypeName(slot.getValueType())
                                            + ") )");
                        }
                    }
                    buffer.append("\n)");
