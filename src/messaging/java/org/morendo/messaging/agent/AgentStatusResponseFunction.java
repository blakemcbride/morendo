...
/**
 * AgentStatusResponse prints the information from an agent when a ping is sent by Morendo. The
 * function calculates the elapsed time by subtracting the sent time from the current time. It also
 * prints the time it took to send the ping message to the agent. This allows us to see if there are
 * any delays in the network.
 *
 * <p>The function prints out the message and returns a string representing the information. This is
 * so that we can get that information and display it in a UI.
 *
 * @author Peter Lin
 */
