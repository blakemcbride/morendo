...
/**
 * PingAgent is similar to DOS/Unix ping command. The function can ping one agent, or all active
 * agents. The message to the agents have a timestamp. The return message contains two timestamps:
 * sent timestamp and received timestamp.
 *
 * <p>The return message from the agent calls agent-status-response function, which prints out the
 * information.
 *
 * @author Peter Lin
 */
public class PingAgentFunction implements Function {
    ...
    public String toPPString(Parameter[] params, int indents) {
        return "("
                + PING_AGENT
                + " <*client instance name*> <ipaddress> <hostname> <application name>)";
    }
}
