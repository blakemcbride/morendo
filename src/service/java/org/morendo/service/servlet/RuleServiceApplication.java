...
import org.morendo.service.ClipsInitialData;
import org.morendo.service.ClipsRuleset;
import org.morendo.service.FunctionPackage;
import org.morendo.service.InitialData;
import org.morendo.service.Model;
import org.morendo.service.ObjectData;
import org.morendo.service.ObjectModel;
import org.morendo.service.RuleApplication;
import org.morendo.service.Ruleset;
...
/**
 * The default implementation of RuleApplication interface. It does not manage or maintain engine
 * pools. Instead, it just defines the configuration properties a rule application may have.
 *
 * @author Peter Lin
 */
public class RuleServiceApplication implements RuleApplication {
    ...
    /**
     * FunctionGroup just lists the names, we keep the instances in a list to make it easier to
     * reload.
     */
    ...
    /**
     * Creates a new URLClassLoader using the URL's from the models, and functions. The initial data
     * depends on the model definitions and should not use any data that isn't declared in the
     * models.
     *
     * @return
     */
    protected URLClassLoader createURLClassLoader() {
        ...
        if (this.functionGroups != null) {
            for (int idx = 0; idx < functionGroups.size(); idx++) {
                org.morendo.service.FunctionPackage functionGroup = functionGroups.get(idx);
    ...
    /**
     * Method will initialize the engine in the following order. 1. the models 2. the function
     * groups 3. rulesets 4. initial data
     */
    ...
    /**
     * The current implementation iterates over the list of models and calls Model.loadModel(Rete).
     */
    ...
    /**
     * The current implementation allows Function and FunctionGroup. This means users can group a
     * variety of functions or groups into a logical group. Within the rule engine, functions will
     * be added to the main group.
     */
    public boolean loadFunctionGroups(Rete engine) {
        ...
        for (int idx = 0; idx < this.functionGroups.size(); idx++) {
            org.morendo.service.FunctionPackage functionGroup = this.functionGroups.get(idx);
            ...
            for (int fx = 0; fx < classnames.length; fx++) {
                ...
                try {
                    ...
                    if (clzz != null) {
                        ...
                        if (data instanceof org.morendo.rete.Function) {
                            Function func = (Function) data;
                            engine.declareFunction(func);
                        } else if (data instanceof org.morendo.rete.FunctionGroup) {
                            org.morendo.rete.FunctionGroup fGroup =
                                    (org.morendo.rete.FunctionGroup) data;
    ...
    /**
     * Reinitialize calls close first to clear the application. Next it reloads the jar files in the
     * classloader and continues.
     */
    ...
    public boolean reloadFunctionGroups(Rete engine) {
        for (int idx = 0; idx < this.functionInstances.size(); idx++) {
            ...
            if (data instanceof org.morendo.rete.Function) {
                ...
                engine.removeFunctionGroup((org.morendo.rete.FunctionGroup) data);
            }
        }
