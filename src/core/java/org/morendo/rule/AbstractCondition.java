...
public abstract sealed class AbstractCondition implements Condition permits ObjectCondition {
    ...
    /** */
    /** The string template name from the parser, before we resolve it to the Template object */
    protected String templateName = null;

    /** the constraints for the condition element */
    protected List<Constraint> constraints = new ArrayList<>(8);

    /** In the case the object pattern is negated, the boolean would be set to true. */
    ...
    /**
     * The potential memory cost based on the maximum partial match count. It is calculated based on
     * the distinct values for each slot.
     */
    ...
    /**
     * by default patterns are not negated. Negated Conditional Elements (aka object patterns) are
     * expensive, so they should be used with care.
     *
     * @return
     */
