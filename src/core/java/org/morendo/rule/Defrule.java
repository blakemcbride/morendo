...
public class Defrule implements Rule, Scope {
    ...
    /** the current implementation simply replaces the existing value if one already exists. */
    ...
    /**
     * This should be called when the action is being fired. after the rule actions are executed,
     * the trigger facts should be reset. The primary downside of this design decision is it won't
     * work well with multi- threaded parallel execution. Since Sumatra has no plans for
     * implementing parallel execution using multi-threading, the design is not an issue.
     * Implementing multi-threaded parallel execution isn't desirable and has been proven to be too
     * costly. A better approach is to queue assert retract and process them in sequence.
     */
    ...
    public void setRuleProperties(List<?> props) {
        ...
        while (itr.hasNext()) {
            ...
            } else if (declaration.getName().equals(RuleProperty.REMEMBER_ALPHA)) {
