...
/**
 * @author Peter Lin
 *     <p>Conditions are patterns. It may be a simple fact pattern, test function, or an object
 *     pattern.
 */
public sealed interface Condition extends Print
        permits AbstractCondition,
                TestCondition,
                AndCondition,
                OrCondition,
                ForallCondition,
    ...
    /**
     * Get the nodes associated with the condition. In the case of TestConditions, it should only be
     * 1 node.
     *
     * @return
     */
    ...
    /**
     * When the rule is compiled, we add the node to the condition, so that we can print out the
     * matches for a given rule.
     *
     * @param node
     */
    ...
    /**
     * if the rule's alpha nodes aren't shared, this method is used to add the alphaNodes to the
     * condition
     *
     * @param node
     */
    ...
    /**
     * Get the bind Constraint List including BoundConstraint (isObjectBinding==false) and
     * PredicateConstraint (isPredicateJoin==true)
     *
     * @return
     */
