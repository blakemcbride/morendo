...
/**
 * @author Peter Lin
 *     <p>The rule interface design is influenced by RuleML 0.8. It is also influenced by CLIPS, but
 *     with some important differences. The interface assumes it acts as a bridge between a Rule
 *     Parser, which parses some text and produces the necessary artifacts and a rule compiler which
 *     generates RETE nodes. For that reason, the interface defines methods for adding Join nodes
 *     and retrieving the last node in the rule. These convienance method are present to make it
...
public interface Rule extends Scope {
    ...
    /**
     * if the rule should fire even when the module is not in focus, call the method with true
     *
     * @param auto
     */
    ...
    /**
     * if users want to give a rule a comment, the method will return it. otherwise it should return
     * zero length string
     *
     * @return
     */
    ...
    /**
     * set the comment of the rule. it should be a descriptive comment about what the rule does.
     *
     * @param text
     */
    ...
    /**
     * get the complexity of the rule, which measure how many conditions a rule has
     *
     * @return
     */
    ...
    /**
     * set the complexity of the rule. this should be calculated by the rule engine or rule compiler
     *
     * @param complexity
     */
    ...
    /**
     * Be default classes implementing the interface should set the effective date to zero. only
     * when the user sets the date should it have a non-zero positive long value.
     *
     * @param mstime
     */
    ...
    /**
     * by default classes implementing the interface should set the expiration date to zero. only
     * when the user sets the date should it have a non-zero positive value greater than the
     * effective date.
     *
     * @param mstime
     */
    ...
    /**
     * if the rule should skip the agenda and fire immediately, the method returns true. By default
     * it should be false
     *
     * @return
     */
    ...
    /**
     * classes implementing the interface can choose to ignore this rule property. Sumatra currently
     * provides the ability to turn off AlphaMemory. By default, it is set to true. If a user wants
     * to turn off AlphaMemory for a given rule, set it to false.
     *
     * @return
     */
    ...
    /**
     * if the rule uses temporal facts, the method should return true. the defrule must declare
     * temporal-activation true
     *
     * @return
     */
    ...
    /**
     * set the cost value for the rule. note that the cost value may change over time. If rules are
     * dynamically added at runtime, the cost value may not be accurate.
     *
     * @param val
     */
    ...
    /**
     * The method should return the last node in the rule, not counting the terminal node.
     *
     * @return
     */
     ...
     * can add the activation to the correct activationList.
     *
     * @param mod
     */
    ...
    /**
     * Return the module the rule belongs to. A rule can only belong to a single module.
     *
     * @return
     */
    Module getModule();
    ...
    /**
     * A rule action can create local bindings, so a rule needs to provide a way to store and
     * retrieve bindings.
     *
     * @param key
     * @param value
    ...
    /**
     * We need to set the trigger facts, so the rule action can look up values easily.
     *
     * @param facts
     */
    ...
    /**
     * In case an user wants to get the trigger facts in the right hand side of the rule.
     *
     * @return
     */
    ...
    /**
     * After the actions of a rule are executed, reset should be called to make sure the rule
     * doesn't hold on to the facts.
     */
    void resetTriggerFacts();

    /**
     * this method needs to be called before rule compilation begins. It avoids doing multiple
     * lookups for the corresponding template.
     *
     * @param engine
     */
