...
import org.morendo.rete.CubeBinding;
import org.morendo.rete.CubeDimension;
import org.morendo.rete.CubeFact;
import org.morendo.rete.CubeHashMemoryImpl;
import org.morendo.rete.Defmeasure;
import org.morendo.rete.EqHashIndex;
import org.morendo.rete.Fact;
import org.morendo.rete.Index;
import org.morendo.rete.ResultsetFact;
...
public class QueryCubeQueryJoin extends QueryBaseJoin {
    ...
    /**
     * Since a Cube ia asserted when it is defined, it will always propagate down the network before
     * other facts. This means when the right input is triggered, we "might" not need to iterate
     * over the left memories. For now it will, but that's one thing we can optimize later on.
     *
     * @param factInstance
     * @param engine
     */
    ...
    protected Object[] queryCube(
            Index linx, Cube c, Rete engine, WorkingMemory mem, ResultsetFact resultFact) {
        ...
        if (this.binds.length > 0) {
            ...
            String parameter =
                    linx.getFacts()[b.getLeftRow()].getSlotValue(b.getLeftIndex()).toString();
     ...
     * @param resultFact
     */
    protected void executeMeasures(
            Cube c, Rete engine, WorkingMemory mem, ResultsetFact resultFact) {
        if (this.measureBindings != null && this.measureBindings.length > 0) {
            for (int idx = 0; idx < this.measureBindings.length; idx++) {
                ...
                if (msr instanceof AggregateMeasure aggrMsr) {
                    calculatedValue =
                            aggrMsr.calculate(engine, c, resultFact.getResultsetData(), cbinding);
