...
import org.morendo.rete.AlphaMemory;
...
public class QueryObjTypeNode extends QueryBaseAlpha {
    ...
    /**
     * It's useful to keep track of the fact count, so that we can use it at execution time. If the
     * rule has joins that involve the template type, but the count is zero we can avoid running the
     * query. It's also good for statistics.
     */
    ...
    /**
     * AssertFact works differently than the normal ObjectTypeNode. Rather than iterate over all the
     * facts in the working memory, the implementation gets the memories for the corresponding
     * ObjectTypeNode. Once it has that, it propagates the facts down the network. A
     * QueryObjTypeNode must have a valid ObjectTypeNode. Since the network will propagate the facts
     * to the ObjectTypeNode, we can avoid iterating over the facts to figure out which ones match.
     *
     * @param fact
     * @param engine
     */
