...
import org.morendo.rete.BetaMemoryImpl;
...
/**
 * NotJoinFrst is a special node for rules that have NOTCE for the first CE. The node does not
 * propogate the initial fact down the network.
 *
 * @author Peter Lin
 */
public class QueryNotJoinFrst extends QueryBaseNot {
    ...
    /**
     * assertLeft for the query node works different than the regular NotJoinFrst node. If there's
     * any memories in the right, it means there's a match. Since the node is NOTCE, we don't
     * propogate. The node only propogates when there's no memories in the right side.
     *
     * @param factInstance
     * @param engine
     */
    ...
    /**
     * When the first conditional element is NOTCE for a query, we simply check to see if there's
     * any memories in the right. If there isn't, we propogate down the network. If there is atleast
     * 1 match, we do not propogate.
     *
     * @param engine
     * @param mem
    ...
    public boolean evaluate(Fact left, int leftId, Fact right, int rightId, Operator opr) {
        if (opr == Operator.NOTEQUAL) {
            return Evaluate.evaluateNotEqual(
                    left.getSlotValue(leftId), right.getSlotValue(rightId));
        } else {
