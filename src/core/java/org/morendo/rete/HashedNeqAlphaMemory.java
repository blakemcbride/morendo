...
/**
 * @author Peter Lin HashedAlphaMemory2 is different in that it has 2 levels of indexing. The first
 *     handles equal to comparisons. The second level handles not equal to.
 */
public class HashedNeqAlphaMemory extends HashedAlphaMemoryImpl {
    ...
    public void clear() {
        ...
        while (itr.hasNext()) {
            Object key = itr.next();
            Map<Object, Object> matches = this.memory.get(key);
    ...
    public int size() {
        ...
        int count = 0;
        while (itr.hasNext()) {
            Map<Object, Object> matches = this.memory.get(itr.next());
            Iterator<?> itr2 = matches.keySet().iterator();
            while (itr2.hasNext()) {
    ...
    /**
     * if there are zero matches for the NotEqHashIndex2, the method return true. If there are
     * matches, the method returns false. False means there's 1 or more matches
     *
     * @param index
     * @return
     */
