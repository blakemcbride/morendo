 ...
 *     <p>Returns the absolute value of a double value.
 */
public class IfFunction implements Function {
    ...
    public ReturnVector executeFunction(Rete engine, Parameter[] params) {
        ...
        DefaultReturnValue rv = new DefaultReturnValue(ValueType.OBJECT, result);
    ...
    public String toPPString(Parameter[] params, int indents) {
        if (params != null && params.length >= 0) {
            ...
            return "(if <boolean expression> then <expression>+ [else <expression>+])\n"
                    ...
                    + "\tExecutes the expressions after then if the boolean expressions"
                    + " evaluates to true, otherwise it executes the expressions after the"
                    + " optional else.";
        }
    }
}
