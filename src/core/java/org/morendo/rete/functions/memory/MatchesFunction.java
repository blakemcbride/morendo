...
/**
 * @author Peter Lin
 *     <p>MatchesFunction will print out all partial matches including alpha and beta nodes.
 */
public class MatchesFunction extends BaseMatchFunction implements Function {
    ...
    /**
     * If the function is called without any parameters, it prints out all the memories. if
     * parameters are passed, the output will be filtered.
     */
    ...
    protected void printAlphaNodeInfo(BaseAlpha alphaNode, Rete engine, DefaultWM wm) {
        if (!(alphaNode instanceof ObjectTypeNode)
                && !(alphaNode instanceof LIANode)
                && !(alphaNode instanceof IFLIANode)) {
