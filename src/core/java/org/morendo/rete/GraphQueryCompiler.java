...
import org.morendo.model.Edge;
import org.morendo.model.Graph;
import org.morendo.model.Node;
import org.morendo.rete.exception.AssertException;
import org.morendo.rete.query.GraphResultNode;
import org.morendo.rete.query.QueryAlphaNode;
import org.morendo.rete.query.QueryAndAlphaNode;
...
import org.morendo.rete.query.QueryIntraFactNode;
import org.morendo.rete.query.QueryLIANode;
import org.morendo.rete.query.QueryObjTypeNode;
import org.morendo.rete.query.QueryOrAlphaNode;
...
/**
 * @author Peter Lin
 *     <p>GraphQueryCompiler handles compiling graph queries, which are different from defqueries.
 *     Graph queries perform pattern matching on nodes and edges. It makes it easy to query graph
 *     data based on the label and type.
 */
public class GraphQueryCompiler implements QueryCompiler {
    ...
    public boolean addQuery(Query query) {
        ...
        if (this.currentQuery.getConditions() != null
                && this.currentQuery.getConditions().length > 0) {
            try {
                ...
                CompileEvent ce =
                        new CompileEvent(this.currentQuery, CompileEvent.Kind.INVALID_RULE);
        ...
        if (conds.length > 1) {
            ...
            for (int idx = 1; idx < conds.length; idx++) {
                ...
                if (idx == 1) {
                    prevCE.getLastNode()
                            .addSuccessorNode(
                                    joinNode, this.engine, this.engine.getWorkingMemory());
                }
