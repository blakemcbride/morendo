...
public class DefaultRuleCompiler implements RuleCompiler {
     ...
     * attach the join nodes to the alpha nodes 3. create the terminal node and attach to the last
     * join node.
     *
     * <p>This means the rule compiler takes a 2 pass approach to compiling rules. At the start of
    ...
    public boolean addRule(Rule rule) {
        ...
        if (!this.validate
            ...
            if (rule.getConditions() != null && rule.getConditions().length > 0) {
                ...
                try {
                    ...
                    for (int idx = 0; idx < conds.length; idx++) {
                        ...
                        // implement in the ObjectConditionCompiler.compile or
                        // ExistConditionCompiler.compile
                        ...
                        if ((con instanceof ObjectCondition)
                            ...
                            if (!(con instanceof ExistCondition)
                                    && !(con instanceof MultipleCondition)) {
                                counter++;
                            }
                        }
                    }
            ...
            engine.writeMessage(
                    "Rule " + rule.getName() + " was not added. ",
                    Constants.DEFAULT_OUTPUT); // $NON-NLS-1$ //$NON-NLS-2$
    ...
    /**
     * The method is responsible for creating the right terminal node based on the settings for the
     * rule.
     *
     * @param rl
     * @return
     ...
     *
     * @see org.morendo.rete.RuleCompiler#addListener(org.morendo.rete.CompilerListener)
     */
     ...
     * @return
     */
    public BaseAlpha2 compileConstraint(
            BoundConstraint cnstr, Template templ, Rule rule, int position) {
        BaseAlpha2 current = null;
     ...
     * @return
     */
    public BaseAlpha compileConstraint(
            PredicateConstraint cnstr, Template templ, Rule rule, int position) {
            ...
            if (ConversionUtils.isPredicateOperatorCode(cnstr.getFunctionName())) {
                ...
                if (f != null) {
                    ...
                    if (f.getReturnType() == ValueType.BOOLEAN_PRIM
                        ...
                        AlphaNodePredConstr node =
                                new AlphaNodePredConstr(engine.nextNodeId(), f, parameters);
    ...
    /**
     * The method is responsible for compiling the string form of the actions to the equivalent
     * functions.
     *
     * @param rule
     * @param acts
