...
/**
 * @author Peter Lin
 *     <p>ObjectTypeNode is the input node for a specific type. The node is created with the
 *     appropriate Class. A couple of important notes about the implementation of ObjectTypeNode.
 *     <ul>
 *       <li>the assertFact method does not check the deftemplate matches the fact. this is because
 *           of inheritance.
 *       <li>WorkingMemoryImpl checks to see if the fact's deftemplate has parents. If it does, it
 *           will keep checking to see if there is an ObjectTypeNode for the parent.
...
public class ObjectTypeNode extends BaseAlpha {
    ...
    /**
     * before the operators included not nill, but for it to work properly we have to first test the
     * slot is not nill, and then do the hash lookup. not sure that it's worth it, so removed it
     * instead.
     */
    public static final Operator[] operators = {Operator.EQUAL, Operator.NILL};
    ...
    /**
     * clear the memory. for now the method does not remove all the successor nodes. need to think
     * it over a bit.
     */
    ...
    /**
     * method to clear the successors. method doesn't iterate over the succesors and clear them
     * individually.
     */
    ...
    /**
     * assert the fact and propogate. ObjectTypeNode does not call assertEvent, since it's not that
     * important and doesn't really help debugging.
     *
     * @param fact
     * @param engine
     */
        ...
        for (int idx = 0; idx < slots.length; idx++) {
            if (slots[idx].getNodeCount() > 0) {
                ...
                CompositeIndex compIndex =
                        new CompositeIndex(slots[idx].getName(), Operator.EQUAL, slotValue);
    ...
    /**
     * Propogate the fact using the normal way of iterating over the successors and calling assert
     * on AlphaNodes and assertRight on BetaNodes.
     *
     * @param fact
     * @param engine
     * @param mem
    ...
    /**
     * Retract the fact to the succeeding nodes. ObjectTypeNode does not call assertEvent, since
     * it's not that important and doesn't really help debugging.
     *
     * @param fact
     * @param engine
     */
    ...
    /**
     * The method returns the successor count taking into consideration the alpha node hashing. It
     * is used by the topology cost function.
     *
     * @return
     */
