...
public abstract class BaseJoin extends BaseNode {
    ...
    /**
     * Subclasses must implement this method. retractLeft takes input from left input adapter nodes
     * and join nodes.
     *
     * @param lfacts
     * @param engine
     */
    public abstract void retractLeft(Index lfacts, Rete engine, WorkingMemory mem)
            throws RetractException;

    /**
     * Subclasses must implement this method. retractRight takes input from alpha nodes.
     *
     * @param rfact
     * @param engine
     */
    ...
    /**
     * When new Successor nodes are added, we propogate the facts that matched to the new join node.
     *
     * @param node
     * @param engine
     * @param mem
    ...
    /**
     * it's unlikely 2 rules are identical, except for the name. The implementation gets the current
     * memory and propogates, but I wonder how much sense this makes in a real production
     * environment. An user really shouldn't be deploying identical rules with different rule name.
     *
     * @param node
     * @param engine
     * @param mem
