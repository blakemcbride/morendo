...
/**
 * @author Peter Lin
 *     <p>BaseNode is meant to define common logic that all nodes must have and implement common
 *     logic.
 */
public abstract class BaseNode implements Print {
    ...
    /**
     * BaseNode has only one constructor which takes an unique node id. All subclasses need to call
     * the constructor.
     */
    ...
    public abstract void addSuccessorNode(BaseNode node, Rete engine, WorkingMemory mem)
    ...
    /** Subclasses need to implement clear and make sure all memories are cleared properly. */
    ...
    /** toString should return a string format of the node and the pattern it matches. */
    public abstract String toString();

    /**
     * hashString should return a string which can be used as a key for HashMap or HashTable
     *
     * @return
     */
    ...
    /**
     * toPPString should return a string format, but formatted nicely so it's easier for humans to
     * read. Chances are this method will be used in debugging mode, so the more descriptive the
     * string is, the easier it is to figure out what the node does.
     *
     * @return
     */
