...
/**
 * @author Peter Lin
 *     <p>FunctionParam is a parameter which gets a value from a nested function call. As a general
 *     rule, a function parameter may need to assert/modify/retract facts; therefore the default
 *     constructor takes Rete. This can happen when a user writes a test pattern like (test (> 10 (*
 *     .5 ?var) ) ) . In the example above, the test would first multiple the bound variable with
 *     .5. The product is then compared to 10. At runtime, the TestNode would pass the necessary
 *     fact to a function that uses a bound variable.
 */
public final class FunctionParam extends AbstractParam {
    ...
    /** reset the function and set the references to the facts to null */
    public void reset() {
        this.facts = null;
        this.value = null;
    }
