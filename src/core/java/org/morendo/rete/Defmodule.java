...
/**
 * @author Peter Lin
 *     <p>A module represents a set of rulesets. The concept is from CLIPS and provides a way to
 *     isolate the rule activation and pattern matching.
 */
public class Defmodule implements Module {
    ...
    /** The name of the module. A rule engine may have one or more modules with rules loaded */
    protected String name = null;

    /**
     * A simple list of the rules in this module. Before an activation is added to the module, the
     * class should check to see if the rule is in the module first.
     */
    ...
    /**
     * The key is either the template name if it was created from the shell, or the defclass if it
     * was created from an Object.
     */
    protected Map<String, Template> deftemplates = null;

    /** we have a second HashMap that maps the class to the deftemplate instance. */
    ...
    /**
     * When the focus is changed, fireActivations should be called to make sure any activations in
     * the module are processed.
     */
    ...
    /**
     * The method should get the agenda and use it to add the new activation to the agenda
     *
     * @param actv
     */
    ...
    /**
     * The current implementation will remove the first activation and return it. If there's no more
     * activations, the method return null;
     */
    ...
    /**
     * When clear is called, the module needs to clear all the internal lists for rules and
     * activations. The handle to Rete should not be nulled.
     */
    public void clear() {
    ...
    /**
     * If the module already contains the rule, it will return true. The lookup uses the rule name,
     * so rule names are distinct within a single module. The same rule name may be used in multiple
     * modules.
     */
    ...
    /**
     * implementation looks up the template and assumes the key is the classname or the user define
     * name.
     */
    public Template getTemplate(Defclass key) {
        Template templ = this.deftemplates.get(key.getClassObject().getName());
        if (templ == null) {
    ...
    /**
     * The implementation will use either the defclass or the template name for the key. The
     * templates are stored in a HashMap.
     */
    ...
    /**
     * implementation will remove the template from the HashMap and it will remove the
     * ObjectTypeNode from the network.
     */
    ...
    /**
     * Method returns the entrySet of the HashMap containing the Deftemplates. Because of how we map
     * the deftemplates, the number of entries will not correspond to the number of actual
     * deftemplates
     */
    ...
    /** Call the method with true to turn on lazy agenda. Call with false to turn it off. */
