...
/**
 * @author Peter Lin
 *     <p>NSFact stands for Non-Shadow Fact. NSFact is different than Deffact which is a shadow fact
 *     for an object instance. NSFact should only be used for cases where fact modification isn't
 *     needed. In all cases where the application expects to modify facts in the reasoning cycle,
 *     Deffacts should be used. Using NSFact for situations where facts are modified or asserted
 *     during the reasoning cycle will produce unreliable results. It will violate the principle of
 *     truth maintenance, which means the final result is true and accurate.
 *     <p>Cases where NSFact is useful are routing scenarios where the facts are filtered to
 *     determien where they should go. In cases like that, the consequence produces results which
 *     are used by the application, but aren't used by the rule engine for reasoning.
 */
public class NSFact implements Fact, Serializable {
    ...
    /**
     * The implementation gets the Defclass and passes the objectInstance to invoke the read method.
     *
     * @see org.morendo.rete.Fact#getSlotValue(int)
     */
    ...
    /**
     * @see org.morendo.rete.Fact#getSlotId(java.lang.String)
     */
    ...
    /**
     * Non-Shadow Fact does not implement this, since this method doesn't apply to facts derived
     * from objects.
     */
    ...
    /** clear will set all the references to null. this makes sure objects are GC. */
    public void clear() {
        this.slots = null;
    ...
    /** non shadow fact does not implement the method, since it doesn't apply. */
