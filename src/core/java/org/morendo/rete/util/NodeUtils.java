...
import org.morendo.rete.BindValue;
import org.morendo.rete.Binding;
import org.morendo.rete.Fact;

public class NodeUtils {
    /**
     * get the values from the left side for nodes that do not have joins with !=
     *
     * @param facts
     * @return
     */
    public static Object[] getLeftValues(Binding[] binds, Fact[] facts) {
        ...
        for (int idx = 0; idx < binds.length; idx++) {
            vals[idx] = facts[binds[idx].getLeftRow()].getSlotValue(binds[idx].getLeftIndex());
        }
        return vals;
    }

    /**
     * convienance method for getting the values based on the bindings for nodes that do not have !=
     *
     * @param ft
     * @return
     */
    ...
    public static BindValue[] getRightBindValues(Binding[] binds, Fact ft) {
        ...
        for (int idx = 0; idx < binds.length; idx++) {
            vals[idx] =
                    new BindValue(
                            ft.getSlotValue(binds[idx].getRightIndex()), binds[idx].negated());
        }
        return vals;
    }
    ...
    public static BindValue[] getLeftBindValues(Binding[] binds, Fact[] facts) {
        ...
        for (int idx = 0; idx < binds.length; idx++) {
            vals[idx] =
                    new BindValue(
                            facts[binds[idx].getLeftRow()].getSlotValue(binds[idx].getLeftIndex()),
                            binds[idx].negated());
        }
        return vals;
    }
