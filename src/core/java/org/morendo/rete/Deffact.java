...
/**
 * @author Peter Lin
 *     <p>Deffact is a concrete implementation of Fact interface. It is equivalent to deffact in
 *     CLIPS.
 */
public class Deffact implements Fact {
    ...
    /**
     * In some cases, a deffact may have bindings. This is a design choice. When rules are parsed
     * and compiled, actions that assert facts are converted to Deffact instances with BoundParam
     * for the slot value.
     *
     * @return
     */
    ...
    public void resolveValues(Rete engine, Fact[] triggerFacts) {
        for (int idx = 0; idx < this.boundSlots.length; idx++) {
            if ((this.boundSlots[idx] instanceof MultiSlot)
                    && !(this.boundSlots[idx].value instanceof BoundParam)) {
    ...
    /**
     * If the fact is a shadow fact, it will return the object instance. If the fact is just a
     * deffact and isn't a shadow fact, it return null.
     *
     * @return
     */
    ...
    /**
     * Returns the string format for the fact without the fact-id. this is used to make sure that if
     * an user asserts an equivalent fact, we can easily check it.
     *
     * @return
     */
    ...
    public int slotHash() {
        ...
        for (int idx = 0; idx < this.slots.length; idx++) {
            if (this.slots[idx] instanceof MultiSlot) {
                hash +=
                        this.slots[idx].getName().hashCode()
    ...
    /**
     * if the factId is -1, the fact will get will the next fact id from Rete and set it. Otherwise,
     * the fact will use the same one.
     *
     * @param engine
     */
    ...
    /**
     * this is used to reset the id, in the event an user tries to assert the same fact again, we
     * reset the id to the existing one.
     *
     * @param fact
     */
    ...
    /**
     * the current implementation only compares the values, since the slot names are equal. It would
     * be a waste of time to compare the slot names. The exception to the case is when a deftemplate
     * is changed. Since that feature isn't supported yet, it's currently not an issue. Even if
     * updating deftemplates is added in the future, the deffacts need to be updated. If the
     * deffacts weren't updated, it could lead to NullPointerExceptions.
     *
     * @param fact
     * @return
     */
    ...
    /**
     * Convenience method for cloning a fact. If a slot's value is a BoundParam, the cloned fact
     * uses the value of the BoundParam.
     *
     * @return
     */
    public Deffact cloneFact() {
        Deffact newfact =
                new Deffact(deftemplate, this.objInstance, deftemplate.cloneAllSlots(), -1);
    ...
    /**
     * this will make sure the fact is GC immediately. Note for shadown facts of objects, retract
     * method does not automatically clear the references. this is so that functions can still use
     * the shadow in the action of the rule after it is retracted from the working memory. Clear
     * will set the id, slots, template, and object instance to null. Once clear is called, the
     * deffact is cleared of all data.
     */
    public void clear() {
