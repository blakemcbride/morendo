...
/**
 * IFLIANode is a special Left-Input Adapater node which has alpha memory. LIANode has no memory to
 * make the engine more efficient, but the left input adapter for the InitialFact needs the memory
 * to make sure rules that start with NOTCE work properly. If we don't, the user has to execute
 * (reset) function, so the rule will fire correctly.
 *
 * @author woolfel
 */
