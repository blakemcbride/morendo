...
/**
 * @author Peter Lin
 *     <p>OnlyPredJoin is special case of exists when there's only 1 match and the constraint uses a
        ...
        while (itr.hasNext()) {
            ...
            if (this.evaluate(bmem.getLeftFacts(), rfact, engine)) {
                ...
                if (bmem.matchCount() == 1) {
                    ...
                    try {
                        this.propagateRetract(
                                bmem.getIndex().add(bmem.getOnlyMatch()), engine, mem);
