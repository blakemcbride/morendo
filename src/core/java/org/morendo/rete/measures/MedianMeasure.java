...
/**
 * Median is the value in the middle of a set of numbers. It is different than mean, which is
 * commonly known as average. If the list has an even number of elements, the median is the average
 * of the two center values. If the size is odd, the median is the value in the middle.
 *
 * @author Peter Lin
 */
    ...
    public BigDecimal calculate(Rete engine, Cube cube, Object[] data, CubeBinding binding) {
        if (data != null) {
            ...
            for (int idx = 0; idx < data.length; idx++) {
                ...
                BigDecimal value =
                        (BigDecimal)
                                facts.getFacts()[binding.getLeftRow()].getSlotValue(
                                        binding.getLeftIndex());
                values.add(value);
            }
