...
import org.morendo.rete.macro.PropertyMacros;
import org.morendo.rete.macro.ReadMacro;
import org.morendo.rete.macro.WriteMacro;
import org.morendo.rete.util.ReflectionUtil;
...
/**
 * Defclass contains the introspection information for a single object type. It takes a class and
 * uses java introspection to get a list of the get/set attributes. It also checks to see if the
 * class implements java beans propertyChangeListener support. If it does, the Method object for
 * those two are cached.
 */
public final class Defclass {
    ...
    /**
     * init is responsible for checking the class to make sure it implements
     * addPropertyChangeListener(java.beans.PropertyChangeListener) and
     * removePropertyChangeListener(java.beans.PropertyChangeListener). We don't require the classes
    ...
    protected void getUtilMethods() {
        try {
            ...
            // declared methods.
            addListener =
                    this.OBJECT_CLASS.getMethod(
                            Constants.PCS_ADD, new Class<?>[] {PropertyChangeListener.class});
            removeListener =
                    this.OBJECT_CLASS.getMethod(
                            Constants.PCS_REMOVE, new Class<?>[] {PropertyChangeListener.class});
    ...
    /**
     * Method checks the MethodDescriptor to make sure it only has 1 parameter and that it is a
    ...
    /**
     * If the class has a method for adding propertyChangeListener, the method return true.
     *
     * @return
     */
    ...
    /**
     * Note: haven't decided if the method should throw an exception or not. Assuming the class has
     * been declared and the defclass exists for it, it normally shouldn't encounter an exception.
     * Cases where it would is if the method is not public. We should do that at declaration time
     * and not runtime.
     *
     * @param col
     * @param data
    ...
    public Template createDeftemplate(String tempName) {
        ...
        for (int idx = 0; idx < st.length; idx++) {
            if (this.PROPS[idx].getPropertyType().isArray()) {
                ...
                st[idx].setValueType(
                        ConversionUtils.getTypeCode(this.PROPS[idx].getPropertyType()));
    ...
    /**
     * Create the Deftemplate for the class, but with a given parent. If a template has a parent,
     * only call this method. If the other method is called, the template is not gauranteed to work
     * correctly.
     *
     * @param tempName
     * @param parent
    ...
    /**
     * Method is specifically used to declare Cubes, so that we can assert a cube and use it for
     * pattern matching.
     *
     * @param cube
     * @return
    ...
    /**
     * the purpose of this method is to re-order the PropertyDescriptors, so that template
    ...
    /**
     * Get the addPropertyChangeListener(PropertyChangeListener) method for the class.
     *
     * @return
     */
    ...
    /**
     * Get the removePropertyChangeListener(PropertyChangeListener) method for the class.
     *
     * @return
     */
    ...
    /**
     * Method is responsible for creating instances of the macros and creating an Array of
     * PropertyMacros. If it is successful, it will set the use macro flag to true and set the
     * macros. If it failed for any reason, use macros will stay off.
     */
    ...
    /**
     * Method will make a copy and return it. When a copy is made, the Method classes are not
     * cloned. Instead, just the HashMap is cloned.
     *
     * @return
     */
