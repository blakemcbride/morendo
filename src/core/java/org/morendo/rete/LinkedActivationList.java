...
/**
 * @author Peter Lin
 *     <p>LinkedActivationList is a container for LinkedActivation. It provdes the logic for
 *     modifying a LinkedList created from LinkedActivation. Null values are not permitted, and are
 *     silently ignored. Generally speaking, it doesn't make sense to add a null activation to the
 *     agenda.
 */
public class LinkedActivationList extends AbstractActivationList {
    ...
    /**
     * the sort method uses binary search to find the correct insertion point for the new
     * activation. It's much faster than the brute force method.
     *
     * @param newact
     */
        ...
        if (this.stratey.compare(newact, this.last) >= 0) {
            ...
            while (!added) {
                if (counter <= 1) {
                    ...
                    added = true;
                } else if (this.stratey.compare(newact, cur) >= 0) {
    ...
    /**
     * method will loop for the given count and return the item before it. for example: 1 2 3 4 5 6
    ...
    /**
     * method will loop for the given count and return the item after it. for example: 1 2 3 4 5 6
    ...
    /**
     * removeActivation will check to see if the activation is the first or last before removing it.
     */
    ...
    /**
     * the current implementation iterates over the LinkedActivations from the start until it finds
     * a match. If it doesn't find a match, the method returns false.
     */
    ...
    /**
     * Iterate over the LinkedList and null the references to previous and next in the
     * LinkedActivation
     */
    public void clear() {
    ...
    /**
     * Current implemenation will return the index of the activation, if it is in the LinkedList. If
     * activation isn't in the list, the method returns -1.
     */
    ...
    public ActivationList clone() {
        final LinkedActivationList la = new LinkedActivationList();
        la.count = this.count;
