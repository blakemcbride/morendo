...
import org.morendo.parser.clips.TokenMgrError;
...
public class StringChannelImpl extends AbstractCommunicationChannel implements StringChannel {
    ...
    public void executeCommand(String commandString, boolean blocked) {
        ...
        List<MessageEvent> commandMessages = blocked ? new LinkedList<MessageEvent>() : null;
        ...
        try {
            ...
            while ((command = parser.basicExpr()) != null) {
                ...
                if (blocked) {
                    try {
                        while (blocked) {
                            router.fillMessageList(getChannelId(), commandMessages);
                            int count = commandMessages.size();
                            if (count > 0) {
                                MessageEvent lastMessage = commandMessages.get(count - 1);
            ...
            router.postMessageEvent(
                    new MessageEvent(MessageEvent.Type.PARSE_ERROR, e, getChannelId()));
