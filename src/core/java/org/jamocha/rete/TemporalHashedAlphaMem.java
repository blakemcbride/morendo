...
/**
 * @author Peter Lin
 *     <p>TemporalHashedAlphaMem is the hashed alpha memory for Temporal nodes. We use a
 *     linkedHashMap so that we can easily remove the expired facts. This means at the top of the
 *     list are older facts and the bottom has the newer facts. We only need to remove facts that
 *     are older than the current timestamp - time window. The equal would be this.
 *     <p>if ( (currentTime - time window) > fact.timestamp )
 *     <p>Rather than keep a timestamp of when the fact entered the join node, we assume the elapsed
 *     time between the time the fact entered the engine and when it activated the node is less than
 *     1 second. Keeping a timestamp of when the fact activated the node is too costly and isn't
 *     practical. This means for each fact, there would be n timestamps, where n is the number of
 *     temporal nodes for the given object type.
 *     <p>If we look at the number of temporal node timestamps the engine would need to maintain
 *     would be this.
 *     <p>f * n = number of temporal timestamps
 *     <p>f = number of facts n = number of temporal nodes
 *     <p>If we have 100,000 facts and 100 temporal nodes, the engine would maintain 10,000,000
 *     timestamps. clearly that isn't scalable and would have a significant impact.
 */
