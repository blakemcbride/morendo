...
/**
 * @author Peter Lin
 *     <p>MLTerminalNode provides Modication logic functionality.
 */
public class MLTerminalNode extends TerminalNode2 {
    ...
    /**
     * The implementation checks to see if the rule is active before it tries to assert the fact. It
     * checks in the following order. 1. is the expiration date greater than zero 2. is the current
     * time > the effective date 3. is the current time < the expiration date
     *
     * @param facts
     * @param engine
     */
    public void assertFacts(Index inx, Rete engine, WorkingMemory mem) {
        ...
        if (this.theRule.getEffectiveDate() > 0 && this.theRule.getExpirationDate() > 0) {
            if (time < this.theRule.getEffectiveDate() || time > this.theRule.getExpirationDate()) {
                add = false;
            }
        }
        if (add) {
    ...
    /**
     * The implementation is similar to TerminalNode3 and checks the effective and expiration date.
     * If the activation is not null, it means it is still in the agenda and hasn't been executed
     * yet. In that case, we remove the activation. If it isn't in the agenda, it means it already
     * fired. that means we need to add ModificationActivation, which executes the corrective action
     * defined by the rule.
     *
     * @param facts
     * @param engine
