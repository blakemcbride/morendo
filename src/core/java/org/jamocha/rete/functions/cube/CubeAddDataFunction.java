...
/**
 * CubeAddData function is used to update a cube and add more data to it. Unlike other facts,
 * updating a cube does not result in retract + assert. This is done for performance reasons. If we
 * were to do a retract + assert, it could result in rules firing more than needed.
 *
 * <p>One important thing is the engine should generate the rule for updating cubes. The priority of
 * the update cube rules should always fire before other rules. That should be done by setting the
 * salience to 1000 or a value sufficiently high.
 *
 * @author Peter Lin
 */
