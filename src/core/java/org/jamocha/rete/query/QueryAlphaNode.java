...
/**
 * @author Peter Lin
 *     <p>QueryAlphaNode is an alpha node for defqueries. Like no memory alpha nodes, it does not
 *     have any memory. This is because queries are fundamentally different and don't need alpha
 *     memories.<br>
 *     <br>
 */
public class QueryAlphaNode extends QueryBaseAlphaCondition {
    ...
    public boolean evaluate(Fact factInstance) {
        return Evaluate.evaluate(
                this.operator, factInstance.getSlotValue(this.slot.getId()), this.slot.getValue());
    }
    ...
    public CompositeIndex getHashIndex() {
        if (compIndex == null) {
            compIndex = new CompositeIndex(slot.getName(), this.operator, slot.getValue());
        }
        return compIndex;
    }
