...
/**
 * QueryFuncAlphaNode is similar to AlphaNode with the difference that this node calls a function.
 * The function must return boolean type. In other words, the function has to evaluate to true or
 * false. for <, >, <=, >= operator, it's handled by a different node
 *
 * @author Peter Lin
 */
public class QueryFuncAlphaNode extends QueryBaseAlphaCondition {
    ...
    public QueryBaseAlpha clone(Rete engine, Defquery query) {
        ...
        QueryFuncAlphaNode clone =
                new QueryFuncAlphaNode(
                        engine.nextNodeId(), this.function, cloneParams, (Slot) this.slot.clone());
